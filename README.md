...
- 初始投入槓桿比只用於開始日建立第一筆持倉，不作為後續規則的回退比例。
- 預設讓利潤奔騰：新高、回撤與反彈規則設定最低持倉底線，實際槓桿權重較高時不賣出。
- 強制再平衡預設永不；需要精確調回當時底線時，可選 30／180／365 或自訂 N 日曆天週期。
...
歷史基準檔採用 FinMind 的 TWSE 衍生批次資料，並以證交所官方日成交與分割公告校驗；後續每月只向 TWSE 補抓缺少月份。`market-raw.json` 保留未調整 OHLC 與配息，`market.json` 則轉換成最新受益權單位基準，避免 0050 4:1、00631L 22:1 分割被誤判為暴跌。資料畫面會顯示必要截止日、各標的最新日期與筆數。
...
- 批次歷史快照雖經 TWSE 官方錨點與企業行動驗證，正式決策前仍應與券商或交易所資料再次交叉核對。
